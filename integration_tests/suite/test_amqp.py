...
def test_stasis_amqp_events(ari):
    res = calling(ari.wazo.createChannel).with_args()
    expected = raises(HTTPError).matching(has_property('response', has_property('status_code', 204)))  # 200 ???
    assert_that(res, expected)
